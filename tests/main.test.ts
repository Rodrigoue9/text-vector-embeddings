...
describe("Plugin tests", () => {
	...
	it("When a comment is created it should add it to the database", async () => {
		...
		const comment = (await context.adapters.supabase.comment.getComment(
			"sasasCreate",
		)) as unknown as CommentMock;
	...
	it("When a comment is updated it should update the database", async () => {
		...
		const comment = (await supabase.comment.getComment(
			updateId,
		)) as unknown as CommentMock;
		expect(comment).toBeDefined();
		expect(comment?.embedding).toBeDefined();
	...
	it("When a comment is deleted it should delete it from the database", async () => {
		...
		const commentBefore =
			await context.adapters.supabase.comment.getComment("sasasDelete");
		...
		expect(
			context.adapters.supabase.comment.getComment("sasasDelete"),
		).rejects.toThrow("Comment does not exist");
	});

	it(
		"When an issue is created with similarity above warning threshold but below match threshold, it should" +
			" update the issue body with footnotes",
		async () => {
			const [warningThresholdIssue1, warningThresholdIssue2] =
				fetchSimilarIssues("warning_threshold_75");
			...
			context.adapters.supabase.issue.findSimilarIssues =
			...
			const { context: context2 } = createContextIssues(
			...
			context2.adapters.supabase.issue.findSimilarIssues =
				mock().mockResolvedValue([
			...
			context2.octokit.rest.issues.update = mock(
				async (params: {
					...
					const updatedBody =
						warningThresholdIssue2.issue_body.replace(
							STRINGS.SIMILAR_ISSUE_TITLE,
							`${STRINGS.SIMILAR_ISSUE_TITLE}[^01^]`,
						) +
	...
	it("When an issue is created with similarity above match threshold, it should close the issue and add a caution alert", async () => {
		const [matchThresholdIssue1, matchThresholdIssue2] =
			fetchSimilarIssues("match_threshold_95");
		const { context } = createContextIssues(
	...
	it("When issue matching is triggered, it should suggest contributors based on similarity", async () => {
		...
		context.octokit.graphql = mock().mockResolvedValue({
			node: {
				...
				assignees: {
					nodes: [
						{ login: "contributor1", url: "https://github.com/contributor1" },
					],
				},
			},
		}) as unknown as typeof context.octokit.graphql;

		context.octokit.rest.issues.createComment = mock(
			async (params: {
				owner: string;
				repo: string;
				issue_number: number;
		...
		const comments = db.issueComments.findMany({
			where: { node_id: { equals: "task_complete" } },
		});
	...
	it("When issue matching is triggered with alwaysRecommend enabled, it should suggest contributors regardless of similarity", async () => {
		...
		context.octokit.graphql = mock().mockResolvedValue({
			node: {
				...
				assignees: {
					nodes: [
						{ login: "contributor3", url: "https://github.com/contributor3" },
					],
				},
			},
		}) as unknown as typeof context.octokit.graphql;
		...
		const comments = db.issueComments.findMany({
			where: { node_id: { equals: "task_complete_always" } },
		});
		expect(comments.length).toBe(1);
		expect(comments[0].body).toContain(STRINGS.CONTRIBUTOR_SUGGESTION_TEXT);
	...
	it("When an issue contains markdown links, footnotes should be added after the entire line", async () => {
		...
		const issue = db.issue.findFirst({
			where: { node_id: { equals: "markdown2" } },
		}) as unknown as Context["payload"]["issue"];
		expect(issue.state).toBe("open");
	...
	it("When a user uses default annotate command and last comment has similarity above annotate threshold, it should update comment body with footnotes", async () => {
		...
		context.adapters.supabase.issue.findSimilarIssues =
			mock().mockResolvedValue([]);
		context.adapters.supabase.comment.findSimilarComments =
			mock().mockResolvedValue([]);
		context.adapters.supabase.issue.createIssue = mock(async () => {
			createIssue(
				annotateIssue.issue_body,
		...
		const { context: context2, comment } = createContext(
			"/annotate",
			1,
			1,
			2,
		...
		context2.adapters.supabase.comment.findSimilarComments =
			mock().mockResolvedValue([]);
		context2.octokit.graphql = mock().mockResolvedValue({
			node: {
				__typename: "Issue",
		...
		context2.octokit.rest.issues.updateComment = mock(
			async (params: {
				owner: string;
				repo: string;
				comment_id: number;
				...
				// Find the most similar sentence (first sentence in this case)
				const updatedBody =
					annotateComment.body.replace(
						STRINGS.SIMILAR_COMMENT,
						`${STRINGS.SIMILAR_COMMENT}[^01^]`,
					) +
		...
		const updatedComment = db.issueComments.findFirst({
			where: { id: { equals: 1 } },
	...
	it("When demoFlag is true, it should skip storing issues in the database", async () => {
		...
		expect(
			context.adapters.supabase.issue.getIssue("demoIssue"),
		).resolves.toBeNull();
	});
	...
	it("When demoFlag is true, it should skip storing comments in the database", async () => {
		...
		expect(
			context.adapters.supabase.comment.getComment("demoComment"),
		).rejects.toThrow();
	});
	...
	it("When demoFlag is false (default), it should store issues in the database", async () => {
		...
		const issue = (await context.adapters.supabase.issue.getIssue(
			"normalIssue",
		)) as unknown as IssueMock;
	...
	function createContext(
		...
		const repo = db.repo.findFirst({
			where: { id: { equals: repoId } },
		}) as unknown as Context["payload"]["repository"];
		const sender = db.users.findFirst({
			where: { id: { equals: payloadSenderId } },
		}) as unknown as Context["payload"]["sender"];
		const issue1 = db.issue.findFirst({
		...
		context.adapters = createMockAdapters(
			context,
	...
	function createContextInner(
		...
		return {
			...
			env: {
				...
				EMBEDDINGS_QUEUE_ENABLED: "false",
			} as Env,
			octokit: octokit,
		};
	}
