...
} from "../src/handlers/pull-request-review-utils";
...
describe("buildPullRequestReviewMarkdown", () => {
	it("combines review body, state, and PR title", () => {
		const result = buildPullRequestReviewMarkdown(
			{ body: "Ship it.", state: "approved" },
			{ title: "Add embeddings", body: "Details here" },
		);
...
describe("formatReviewThreadContext", () => {
	it("adds a reply header and separator", () => {
		const result = formatReviewThreadContext(
			"Child comment",
			"Parent context",
			"alice",
		);
