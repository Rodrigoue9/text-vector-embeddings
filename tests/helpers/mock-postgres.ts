import {
	...
	PostgresPool,
	QueryResult,
} from "../../src/adapters/postgres-driver";
...
function splitRepositoryKey(repositoryKey: string): {
	owner: string;
	repo: string;
} {
...
function getTrackedIssueRows(state: MockPostgresState): TrackedIssueRecord[] {
	return Array.from(state.trackedIssues.entries())
		.flatMap(([repositoryKey, issueNumbers]) => {
			...
			return Array.from(issueNumbers.values()).map((issueNumber) => ({
				owner,
				repo,
				issueNumber,
			}));
...
function createQueryHandler(state: MockPostgresState) {
	return async function queryObject<T>(
		...
		if (
			sql.startsWith(
				"SELECT issue_number FROM text_vector_embeddings_tracked_issues",
			)
		) {
		...
		if (sql.startsWith("INSERT INTO text_vector_embeddings_tracked_issues")) {
			...
			const issueNumbers =
				state.trackedIssues.get(repositoryKey) ?? new Set<number>();
		...
		if (
			sql.startsWith(
				"SELECT owner, repo, array_agg(issue_number ORDER BY issue_number) AS issue_numbers FROM text_vector_embeddings_tracked_issues",
			)
		) {
			const groupedRows = Array.from(state.trackedIssues.entries())
				...
				.map(([repositoryKey, issueNumbers]) => {
					...
					return {
						owner,
						repo,
						issue_numbers: Array.from(issueNumbers.values()).sort(
		...
		if (
			sql.startsWith(
				"SELECT total_hits, reset_time FROM text_vector_embeddings_rate_limit_records",
			)
		) {
		...
		if (
			sql.startsWith("DELETE FROM text_vector_embeddings_rate_limit_records")
		) {
			const key = String(args[0]);
		...
		if (
			sql.startsWith("INSERT INTO text_vector_embeddings_rate_limit_records")
		) {
			const key = String(args[0]);
