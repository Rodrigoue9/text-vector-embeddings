...
type OctokitConstructor =
	typeof import("@ubiquity-os/plugin-sdk/octokit")["customOctokit"];
type OctokitInstance = InstanceType<OctokitConstructor>;
type GetAuthenticatedOctokitArgs = Parameters<
	typeof import("../src/cron/workflow")["getAuthenticatedOctokit"]
>[0];
...
function createMatchResult(): MatchResult {
	return {
		...
		sortedContributors: [
			{ login: "dev", matches: ["body"], maxSimilarity: 0.9 },
		],
	};
}
...
function setupRoute(options: SetupOptions = {}) {
	...
	const issueMatchingForUsersMock = mock(
		async (ctx: IssueOpenedContext, users: string[]) => {
			void ctx;
			void users;
			return options.issueMatchingResult ?? createMatchResult();
		},
	...
	const issuesGetMock = mock(async () => ({
		data: { id: "issue", number: 7, title: "Issue", body: "body" },
	}));
	...
	const getAuthenticatedOctokitMock = mock(
		async (args: GetAuthenticatedOctokitArgs) => {
			void args;
	...
	return {
		recommendationsRoute: createRecommendationsRoute({
			...
			getAuthenticatedOctokit: (args: GetAuthenticatedOctokitArgs) =>
				getAuthenticatedOctokitMock(args),
			initAdapters: (ctx: IssueOpenedContext) => initAdaptersMock(ctx),
			issueMatching: (ctx: IssueOpenedContext) => issueMatchingMock(ctx),
			issueMatchingForUsers: (ctx: IssueOpenedContext, users: string[]) =>
				issueMatchingForUsersMock(ctx, users),
		}),
		mocks: {
			getEnv,
			issueMatchingMock,
...
describe("/recommendations route", () => {
	...
	it("returns issue matching results when app credentials are provided", async () => {
		const matchResult: MatchResult = {
			...
			sortedContributors: [
				{ login: "octokit", matches: ["text"], maxSimilarity: 0.92 },
			],
		};
		...
		expect(mocks.getAuthenticatedOctokitMock).toHaveBeenCalledWith({
			appId: "123",
			appPrivateKey: "key",
			owner: "foo",
			repo: "bar",
		...
		expect(mocks.issuesGetMock).toHaveBeenCalledWith({
			owner: "foo",
			repo: "bar",
			issue_number: 42,
		});
	...
	it("returns a result for every requested user", async () => {
		const matchResult: MatchResult = {
			matchResultArray: {
				alice: [
					"> `80% Match` [foo/bar#1](https://www.github.com/foo/bar/issues/1)",
				],
				bob: [],
			},
