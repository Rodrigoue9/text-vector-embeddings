...
export const handlers = [
	...
	http.get(
		...
		({ params: { org } }: { params: { org: string } }) =>
			HttpResponse.json(
				db.repo.findMany({ where: { owner: { login: { equals: org } } } }),
			),
	),
	// get org repo issues
	http.get(
	...
	http.get(
		...
		({ params: { username } }) =>
			HttpResponse.json(
				db.users.findFirst({
					where: { login: { equals: username as string } },
				}),
			),
	),
	// get repo
	http.get(
		...
		({
			...
			const item = db.repo.findFirst({
				where: { name: { equals: repo }, owner: { login: { equals: owner } } },
			});
			if (!item) {
	...
	http.post(
		...
		async ({ params: { issue_number: issueNumber }, request }) => {
	...
	http.patch(
		...
		async ({ params: { comment_id: commentId }, request }) => {
			const { body } = await getValue(request.body);
			const item = db.issueComments.findFirst({
	...
	http.patch(
		...
		async ({ params: { issue_number: issueNumber }, request }) => {
			const { body } = await getValue(request.body);
			const item = db.issue.findFirst({
				where: { number: { equals: Number(issueNumber) } },
			});
			if (!item) {
				return new HttpResponse(null, { status: 404 });
			}
	...
	http.get(
		...
		({ params: { issue_id: issueId } }) =>
			HttpResponse.json(
				db.issueComments.findMany({
					where: { issue_id: { equals: String(issueId) } },
				}),
			),
	),
];
