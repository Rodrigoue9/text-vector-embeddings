...
import {
	Comment,
	CommentData,
} from "../../src/adapters/supabase/helpers/comment";
		...
		supabase: {
			comment: {
				createComment: jest.fn(async (commentData: CommentData) => {
					...
					const cleanedMarkdown = commentData.markdown
						? stripHtmlComments(commentData.markdown).trim()
						: "";
					const embeddingSource = commentData.isPrivate ? "" : cleanedMarkdown;
					const embedding =
						await context.adapters.voyage.embedding.createEmbedding(
							embeddingSource,
						);
			...
			issue: {
				...
				findSimilarIssuesToMatch: jest.fn(
					async (params: {
						markdown: string;
						threshold: number;
						currentId: string;
