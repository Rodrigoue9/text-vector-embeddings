...
function nextDelayIncrease(current: number, min: number, max: number): number {
	return clampDelay(
		quantizeUp(current + DELAY_STEP_MS, DELAY_STEP_MS),
		min,
		max,
	);
}
...
function nextDelayDecrease(current: number, min: number, max: number): number {
	return clampDelay(
		quantizeDown(current - DELAY_STEP_MS, DELAY_STEP_MS),
		min,
		max,
	);
}
	...
	while (true) {
		...
		if (isTokenLimited) {
			...
			if (!options.autoBatch) {
				logger.error(
					"Token limit hit while auto-batch is disabled. Reduce EMBEDDINGS_QUEUE_BATCH_SIZE or pass --auto-batch.",
					{
						batchSize,
					},
				);
			...
			if (batchSize <= 1) {
				logger.error("Token limit hit at batch size 1; cannot auto-recover.", {
					batchSize,
				});
				process.exit(1);
			}
			...
			nextBatchSize = Math.max(
				batchFloor,
				Math.floor((batchFloor + batchCeil) / 2),
			);
			if (nextBatchSize >= batchSize) {
				logger.error(
					"Token limit hit but batch size cannot shrink further. Lower EMBEDDINGS_QUEUE_BATCH_SIZE.",
					{
						batchSize,
						batchFloor,
						batchCeil,
			...
			if (!options.fixedDelay) {
				...
				if (delayGood !== null && delayGood < delayBad) {
					const candidate = quantizeUp(
						(delayGood + delayBad) / 2,
						DELAY_STEP_MS,
					);
					nextDelayMs = clampDelay(
						Math.max(
							candidate,
							nextDelayIncrease(delayGood, delayMin, delayMax),
						),
						delayMin,
						delayMax,
					);
					...
					nextDelayMs = clampDelay(
						candidate === delayMs
							? nextDelayIncrease(delayMs, delayMin, delayMax)
							: candidate,
						delayMin,
						delayMax,
					);
		...
		if (!result.stoppedEarly && !isTokenLimited) {
			if (!hasProgress && processed === 0) {
				...
				if (emptyPasses >= options.maxEmpty) {
					logger.error(
						"No progress detected; aborting to avoid an infinite loop.",
						{
							pendingAfter,
							emptyPasses,
						},
			...
			if (processed > 0) {
				if (options.autoBatch) {
					if (hasBatchFailure && batchFloor < batchCeil) {
						...
						nextBatchSize = Math.max(
							batchFloor,
							Math.floor((batchFloor + batchCeil + 1) / 2),
						);
				...
				if (!options.fixedDelay) {
					...
					if (delayBad !== null && delayGood < delayBad) {
						...
						nextDelayMs = clampDelay(
							Math.min(
								candidate,
								nextDelayDecrease(delayBad, delayMin, delayMax),
							),
							delayMin,
							delayMax,
						);
