...
import {
	cleanMarkdown,
	isTooShort,
	MIN_ISSUE_MARKDOWN_LENGTH,
} from "../src/utils/embedding-content";
...
function extractRepoFullName(payload: unknown): string {
	...
	const owner =
		ownerObj && typeof ownerObj.login === "string" ? ownerObj.login.trim() : "";
	if (!owner || !name) return "";
	return `${owner}/${name}`;
}

async function listReposForOrg(
...
async function loadExistingDocs(
	...
	byRepo: Map<string, Map<string, ExistingDoc>>;
	byId: Map<string, ExistingDoc>;
}> {
	const perPage = 1000;
	let offset = 0;
	...
	while (true) {
		...
		for (const row of data) {
			...
			if (!fullName || (targetRepos.size > 0 && !targetRepos.has(fullName)))
				continue;
...
function shouldProcessRecord(
	...
	const existingTs = existing?.modifiedAt
		? Date.parse(existing.modifiedAt)
		: Number.NaN;
...
async function fetchIssuePage(
	...
	const response = await octokit.graphql<{
		repository: { issues: { nodes: IssueNode[]; pageInfo: PageInfo } };
	}>(ISSUE_QUERY, {
		owner,
		repo,
		after,
...
async function fetchPullPage(
	...
	const response = await octokit.graphql<{
		repository: {
			pullRequests: { nodes: PullRequestNode[]; pageInfo: PageInfo };
		};
	}>(PULL_QUERY, {
		owner,
		repo,
...
async function upsertDocument(
	...
	const { error } = await supabase
		.from("documents")
		.upsert(
			record as unknown as Database["public"]["Tables"]["documents"]["Insert"],
			{ onConflict: "id" },
		);
	if (error) {
...
async function updateDocument(
	...
	const { error } = await supabase
		.from("documents")
		.update(
			updates as unknown as Database["public"]["Tables"]["documents"]["Update"],
		)
		.eq("id", nodeId);
	if (error) {
...
async function processIssues(params: {
	...
	const { octokit, supabase, repoInfo, existing, options, voyageEmbedding } =
		params;
	...
	while (true) {
		const { nodes, pageInfo } = await fetchIssuePage(
			octokit,
		...
		for (const node of nodes) {
			...
			if (
				...
				Date.parse(updatedAt) < options.since
			) {
				skipped += 1;
				continue;
			}
			...
			const shouldProcess = shouldProcessRecord(
				existingEntry,
				updatedAt,
			...
			const authorId =
				isHuman && node.author?.databaseId ? node.author.databaseId : -1;
			const rawMarkdown = [node.body ?? "", node.title ?? ""]
				.filter(Boolean)
				.join(" ")
				.trim();
...
async function processPullRequests(params: {
	...
	while (true) {
		const { nodes, pageInfo } = await fetchPullPage(
			octokit,
			repoInfo.owner.login,
			repoInfo.name,
			cursor,
		...
		for (const node of nodes) {
			...
			const payload = isPrivate
				? null
				: buildPullRequestPayload(node, repoPayload);
			const record = {
				id: node.id,
...
async function main() {
	...
	if (!token) {
		console.error(
			"Missing GitHub token. Set GITHUB_TOKEN/GH_TOKEN or pass --token.",
		);
		process.exit(1);
	}
	...
	const voyageEmbedding = options.withEmbeddings
		? new VoyageEmbedding(
				new VoyageAIClient({ apiKey: process.env.VOYAGEAI_API_KEY ?? "" }),
				context,
			)
		: null;
	...
	for (const repoFullName of repos) {
		...
		const repoExisting =
			existing.byRepo.get(repoFullName) ?? new Map<string, ExistingDoc>();
		const mergedExisting = new Map<string, ExistingDoc>([
			...existing.byId,
			...repoExisting,
		]);
		...
		if (options.types.has("issue")) {
			...
			console.log(
				`Issues: inserted=${stats.inserted} updated=${stats.updated} skipped=${stats.skipped}`,
			);
		}
		...
		if (options.types.has("pull_request")) {
			...
			console.log(
				`PRs: inserted=${stats.inserted} updated=${stats.updated} skipped=${stats.skipped}`,
			);
		}
	}
}
