...
} from "../src/utils/markdown-comments";
...
async function main() {
	...
	if (!token) {
		console.error(
			"Missing GitHub token. Set GITHUB_TOKEN or GH_TOKEN, or pass --token.",
		);
		process.exit(1);
	}
	...
	for (const issueNumber of issueNumbers) {
		...
		const { cleaned, latestComment, matchCount } =
			stripPluginUpdateComments(body);
		...
		const normalized = options.normalizeWhitespace
			...
			: cleaned;
		const nextBody =
			options.keepLatest && latestComment
				? appendPluginUpdateComment(normalized, latestComment)
				: normalized.trimEnd();
