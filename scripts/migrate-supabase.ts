...
function parseBatchSize(raw: string | undefined): number {
	...
	if (!Number.isFinite(parsed) || parsed <= 0 || parsed > MAX_BATCH_SIZE) {
		console.error(
			`Invalid BATCH_SIZE: ${raw}. Use an integer between 1 and ${MAX_BATCH_SIZE}.`,
		);
		process.exit(1);
	}
	return parsed;
...
async function copyTable({ name, orderBy }: TableSpec): Promise<void> {
	...
	while (true) {
		...
		if (error) {
			throw new Error(
				`Failed to read ${name} at offset ${offset}: ${error.message}`,
			);
		}
		...
		if (isDryRun) {
			...
			if (upsertError) {
				throw new Error(
					`Failed to upsert ${name} at offset ${offset}: ${upsertError.message}`,
				);
			}
		}
	...
	if (!isDryRun && sourceCount !== null && targetCountAfter !== null) {
		if (targetCountAfter < sourceCount) {
			throw new Error(
				`Row count mismatch for ${name}: source=${sourceCount} target=${targetCountAfter}`,
			);
		}
		if (targetCountAfter > sourceCount) {
			console.warn(
				`Target row count exceeds source for ${name}: source=${sourceCount} target=${targetCountAfter}`,
			);
		}
	}
}
