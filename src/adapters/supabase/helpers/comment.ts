...
export class Comment extends SuperSupabase {
	...
	async createComment(
		...
		const isCommandComment = cleanedMarkdown
			? isCommandLikeContent(cleanedMarkdown)
			: false;
		const isShortComment = isTooShort(
			cleanedMarkdown,
			MIN_COMMENT_MARKDOWN_LENGTH,
		);
		...
		this.context.logger.ok(
			`Comment created successfully with id: ${commentData.id}`,
			{ data },
		);
	}
	...
	async updateComment(
		...
		if (comments && comments.length == 0) {
			...
			this.context.logger.ok(
				"Comment updated successfully with id: " + commentData.id,
				{
					commentData: {
						commentData,
						markdown: finalMarkdown,
	...
	async deleteComment(commentNodeId: string) {
		...
		this.context.logger.ok(
			"Comment deleted successfully with id: " + commentNodeId,
		);
	}
	...
	async findSimilarComments({
		...
		currentId,
		threshold,
	}: FindSimilarCommentsParams): Promise<
		CommentSimilaritySearchResult[] | null
	> {
		...
		try {
			...
			if (!embeddingSource) {
				this.context.logger.debug(
					"Skipping comment similarity search because text is empty after stripping comments.",
					{ currentId },
				);
				return null;
			}
			if (isTooShort(embeddingSource, MIN_COMMENT_MARKDOWN_LENGTH)) {
				this.context.logger.debug(
					"Skipping comment similarity search because text is too short.",
					{
						currentId,
