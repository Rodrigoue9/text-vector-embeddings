...
function resolveIssueDocType(
	payload: IssueData["payload"],
	explicitType?: IssueDocumentType,
): IssueDocumentType {
	if (explicitType) {
		return explicitType;
	}
...
export class Issue extends SuperSupabase {
	...
	async createIssue(issueData: IssueData, options: IssueWriteOptions = {}) {
		...
		if (!shouldDeferEmbedding && embeddingSource && !isPrivate) {
			embedding =
				await this.context.adapters.voyage.embedding.createEmbedding(
					embeddingSource,
				);
		}
		...
		this.context.logger.ok(
			`Issue created successfully with id: ${issueData.id}`,
			{ data },
		);
	}
	...
	async updateIssue(issueData: IssueData, options: IssueWriteOptions = {}) {
		...
		if (!issues || issues.length === 0) {
			...
			await this.createIssue(
				...
				{ deferEmbedding: shouldDeferEmbedding },
			);
			return;
		}
		...
		this.context.logger.ok(
			"Issue updated successfully with id: " + issueData.id,
			{
				issueData: {
					id: issueData.id,
					markdown: finalMarkdown,
	...
	async deleteIssue(issueNodeId: string) {
		...
		this.context.logger.ok(
			"Issue deleted successfully with id: " + issueNodeId,
		);
	}
	...
	async findSimilarIssues({
		...
		currentId,
		threshold,
	}: FindSimilarIssuesParams): Promise<IssueSimilaritySearchResult[] | null> {
		// Create a new issue embedding
		try {
			const embeddingSource = cleanMarkdown(markdown);
			if (!embeddingSource) {
				this.context.logger.debug(
					"Skipping issue similarity search because text is empty after stripping comments.",
					{ currentId },
				);
				return null;
			}
			if (isTooShort(embeddingSource, MIN_ISSUE_MARKDOWN_LENGTH)) {
				this.context.logger.debug(
					"Skipping issue similarity search because text is too short.",
					{
						currentId,
	...
	async findSimilarIssuesToMatch({
		markdown,
		currentId,
		threshold,
		topK,
		...
		try {
			...
			if (!embeddingSource) {
				this.context.logger.debug(
					"Skipping issue match search because text is empty after stripping comments.",
					{ currentId },
				);
				return null;
			}
			if (isTooShort(embeddingSource, MIN_ISSUE_MARKDOWN_LENGTH)) {
				this.context.logger.debug(
					"Skipping issue match search because text is too short.",
					{
						currentId,
						length: embeddingSource.length,
						minLength: MIN_ISSUE_MARKDOWN_LENGTH,
