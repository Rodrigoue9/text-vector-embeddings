			...
			async () => {
						...
						messages: [
							{
								role: "system",
								content:
									"You are an assistant that analyzes external images and provides factual descriptions.",
							},
							{
								role: "user",
								content: [
									...
									{
										type: "image_url",
										image_url: { url: `data:image/jpeg;base64,${linkContent}` },
									},
								],
							},
						],
				...
				if (isAsyncIterable(response)) {
					throw this.context.logger.error(
						"Unexpected streaming response from the LLM.",
					);
				}

				if (
					!isChatCompletionLike(response) ||
					!Array.isArray(response.choices)
				) {
					throw this.context.logger.error("Unexpected LLM response shape.", {
				...
				if (typeof content !== "string" || !content.trim()) {
					throw this.context.logger.error(
						"Failed to get a completion from the LLM.",
					);
				}
