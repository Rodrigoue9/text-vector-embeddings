...
import {
	issueMatching,
	issueMatchingForUsers,
} from "../handlers/issue-matching";
...
export function createRecommendationsRoute(
	overrides: Partial<RecommendationsDeps> = {},
) {
	const deps: RecommendationsDeps = {
		getEnv:
			overrides.getEnv ??
			((c) => getValidatedEnv(c) as Context<"issues.opened">["env"]),
		createOctokit: overrides.createOctokit ?? (() => new customOctokit()),
		getAuthenticatedOctokit:
			overrides.getAuthenticatedOctokit ?? getAuthenticatedOctokit,
		initAdapters: overrides.initAdapters ?? initAdapters,
		issueMatching: overrides.issueMatching ?? issueMatching,
		issueMatchingForUsers:
			overrides.issueMatchingForUsers ?? issueMatchingForUsers,
	};
	...
	return async function recommendationsRoute(c: HonoContext) {
		...
		const logger = new Logs(
			"debug",
		) as unknown as Context<"issues.opened">["logger"];
		const honoEnv = deps.getEnv(c);

		function serializeMatchResult(
			result:
				| Awaited<ReturnType<typeof issueMatching>>
				| Awaited<ReturnType<typeof issueMatchingForUsers>>,
		) {
			if (!result) {
				return null;
			}
			const matchResultArray =
				result.matchResultArray instanceof Map
					? Object.fromEntries(result.matchResultArray.entries())
					: result.matchResultArray;
			return {
				...result,
				matchResultArray,
			};
		...
		async function handleUrl(url: string) {
			...
			try {
				...
				if (!appId || !appPrivateKey) {
					logger.warn(
						"APP_ID or APP_PRIVATE_KEY are missing from the env, will use the default Octokit instance.",
					);
				...
				try {
					...
					const result =
						users.length > 0
							? await deps.issueMatchingForUsers(ctx, users)
							: await deps.issueMatching(ctx);
				...
				logger.warn("Failed to process recommendations URL", {
					url,
					error: errorInfo,
				});
				return { [url]: null };
		...
		return new Response(
			JSON.stringify(res.reduce((acc, curr) => ({ ...acc, ...curr }), {})),
