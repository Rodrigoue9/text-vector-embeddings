...
export async function initAdapters(context: Context) {
	...
	context.logger[isConnectionValid ? "ok" : "error"](
		`Supabase connection ${isConnectionValid ? "successful" : "failed"}`,
	);
...
export async function runPlugin(context: Context) {
	...
	try {
		...
		const { enabled: shouldDeferEmbeddings } = getEmbeddingQueueSettings(
			context.env,
		);
		...
		if (isIssueCommentEvent(context)) {
			switch (eventName) {
				case "issue_comment.created":
					...
					return await userAnnotate(
						context as Context<"issue_comment.created">,
					);
				case "issue_comment.deleted":
					return await deleteComment(
						context as Context<"issue_comment.deleted">,
					);
				case "issue_comment.edited":
					return await updateComment(
						context as Context<"issue_comment.edited">,
					);
			}
		} else if (isPullRequestReviewCommentEvent(context)) {
			switch (eventName) {
				case "pull_request_review_comment.created":
					return await addReviewComment(
						context as Context<"pull_request_review_comment.created">,
					);
				case "pull_request_review_comment.edited":
					return await updateReviewComment(
						context as Context<"pull_request_review_comment.edited">,
					);
				case "pull_request_review_comment.deleted":
					return await deleteReviewComment(
						context as Context<"pull_request_review_comment.deleted">,
					);
			}
			...
			switch (eventName) {
				...
				case "pull_request_review.edited":
					return await updatePullRequestReview(
						context as Context<"pull_request_review.edited">,
					);
			}
		} else if (isPullRequestEvent(context)) {
			switch (eventName) {
				case "pull_request.opened":
					return await addPullRequest(
						context as Context<"pull_request.opened">,
					);
				case "pull_request.edited":
					return await updatePullRequest(
						context as Context<"pull_request.edited">,
					);
			}
			...
			switch (eventName) {
				case "issues.opened":
					...
					if (shouldDeferEmbeddings) {
						logger.debug(
							"Embedding queue enabled; skipping issue matching on open.",
						);
					} else {
				...
				case "issues.edited":
					if (isPluginEdit(context as Context<"issues.edited">)) {
						logger.info(
							"Plugin edit detected, will run issue matching and checker.",
						);
						if (shouldDeferEmbeddings) {
							logger.debug(
								"Embedding queue enabled; skipping issue matching and dedupe on plugin edit.",
							);
						} else {
							await issueMatchingWithComment(
								context as Context<"issues.edited">,
							);
			...
			if (shouldDeferEmbeddings) {
				logger.debug(
					"Embedding queue enabled; skipping issue matching on label.",
				);
				return;
			}
			return await issueMatchingWithComment(
				context as Context<"issues.labeled">,
			);
		} else {
