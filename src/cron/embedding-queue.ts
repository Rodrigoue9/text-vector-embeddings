...
import {
	cleanMarkdown,
	isTooShort,
	MIN_COMMENT_MARKDOWN_LENGTH,
	MIN_ISSUE_MARKDOWN_LENGTH,
} from "../utils/embedding-content";
...
function getAuthorType(payload: unknown, docType: DocumentType): string | null {
	...
	if (docType === "pull_request_review") {
		return (
			getNestedString(payload, ["review", "user", "type"]) ??
			getNestedString(payload, ["sender", "type"])
		);
	}

	return (
		getNestedString(payload, ["comment", "user", "type"]) ??
		getNestedString(payload, ["sender", "type"])
	);
}
...
async function preparePendingRow(params: {
	...
	if (authorType && authorType !== "User") {
		const isBotRootReviewAllowed =
			docType === "review_comment" && isReviewCommentThreadRoot(row.payload);
		if (!isBotRootReviewAllowed) {
			logger.debug("Skipping embedding for non-human author.", {
				label,
				id: row.id,
				authorType,
				docType,
			...
			const { error: updateError } = await supabase
				...
				.eq("id", row.id);
			if (updateError) {
				logger.error("Failed to clear markdown for non-human author.", {
					label,
					id: row.id,
					updateError,
				});
		...
		logger.debug("Allowing bot-authored root review comment for embedding.", {
			label,
			id: row.id,
			authorType,
			docType,
	...
	const minLength = isIssueDoc
		? MIN_ISSUE_MARKDOWN_LENGTH
		: MIN_COMMENT_MARKDOWN_LENGTH;
	if (
		(docType === "issue_comment" ||
			docType === "review_comment" ||
			docType === "pull_request_review") &&
		isCommandLikeContent(cleaned)
	) {
		logger.debug("Skipping embedding for command-like comment.", {
			label,
			id: row.id,
			docType,
		});
		const { error: updateError } = await supabase
			...
			.eq("id", row.id);
		if (updateError) {
			logger.error("Failed to clear markdown for command-like comment.", {
				label,
				id: row.id,
				updateError,
			});
	...
	if (!cleaned) {
		logger.debug("Skipping empty markdown embedding.", {
			label,
			id: row.id,
			docType,
		});
		const { error: updateError } = await supabase
			...
			.eq("id", row.id);
		if (updateError) {
			logger.error("Failed to clear markdown for empty content.", {
				label,
				id: row.id,
				updateError,
			});
	...
	if (isTooShort(cleaned, minLength)) {
		logger.debug("Skipping embedding for short content.", {
			label,
			id: row.id,
		...
		const { error: updateError } = await supabase
			...
			.eq("id", row.id);
		if (updateError) {
			logger.error("Failed to clear markdown for short content.", {
				label,
				id: row.id,
				updateError,
			});
...
async function processPendingRows(params: {
	...
	processed: number;
	stoppedEarly: boolean;
	processedByType: Record<DocumentType, number>;
}> {
	...
	const concurrency = Math.max(
		1,
		Math.min(settings.concurrency, updates.length),
	);
	...
	async function worker() {
		while (updates.length > 0) {
			...
			if (!update.embedding.length) {
				logger.error("Embedding batch returned empty vector.", {
					label,
					id: update.row.id,
				});
			...
			const { error: updateError } = await supabase
				.from("documents")
				.update({
					embedding: serializeEmbeddingForDatabase(update.embedding),
...
export async function processPendingEmbeddings(params: {
	...
	const embedder = new VoyageEmbedding(clients.voyage, {
		logger,
	...
	const issuesProcessed =
		(combinedResult.processedByType.issue ?? 0) +
		(combinedResult.processedByType.pull_request ?? 0);
