...
export async function updateCronState(context: Context) {
	...
	if (!process.env.GITHUB_REPOSITORY) {
		context.logger.warn(
			"Can't update the Action Workflow state as GITHUB_REPOSITORY is missing from the env.",
		);
		return;
	}
	...
	try {
		...
		if (
			...
			context.adapters?.supabase?.super?.hasPendingEmbeddings
		) {
			hasPendingEmbeddings =
				await context.adapters.supabase.super.hasPendingEmbeddings();
		}
