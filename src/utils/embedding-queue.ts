...
export function getEmbeddingQueueSettings(env: Env): EmbeddingQueueSettings {
	return {
		enabled: parseBoolean(
			env.EMBEDDINGS_QUEUE_ENABLED,
			isQueueEnabledByDefault,
		),
		batchSize: parsePositiveInt(
			env.EMBEDDINGS_QUEUE_BATCH_SIZE,
			DEFAULT_BATCH_SIZE,
		),
		delayMs: parseNonNegativeInt(
			env.EMBEDDINGS_QUEUE_DELAY_MS,
			DEFAULT_DELAY_MS,
		),
		maxRetries: parseNonNegativeInt(
			env.EMBEDDINGS_QUEUE_MAX_RETRIES,
			DEFAULT_MAX_RETRIES,
		),
		concurrency: parsePositiveInt(
			env.EMBEDDINGS_QUEUE_CONCURRENCY,
			DEFAULT_CONCURRENCY,
		),
	};
}
