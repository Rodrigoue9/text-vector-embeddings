...
const UPDATE_COMMENT_REGEX = new RegExp(
	`<!--\\s*${escapeRegExp(pkg.name)}\\s+update\\s+[^\\n]*?-->`,
	"g",
);

export function appendPluginUpdateComment(
	markdown: string,
	comment: string,
): string {
...
export function stripHtmlComments(markdown: string): string {
	...
	function flushBuffer(preserveComments: boolean) {
		...
		output.push(
			preserveComments ? chunk : chunk.replace(HTML_COMMENT_REGEX, ""),
		);
		buffer = [];
	}
