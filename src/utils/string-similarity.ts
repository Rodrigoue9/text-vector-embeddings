export function findEditDistance(sentenceA: string, sentenceB: string): number {
	...
	const distanceMatrix: number[][] = Array.from({ length: lengthA + 1 }, () =>
		Array.from({ length: lengthB + 1 }, () => 0),
	);
		...
		for (let indexB = 0; indexB <= lengthB; indexB += 1) {
			if (indexA === 0) {
				...
				distanceMatrix[indexA][indexB] =
					1 +
					Math.min(
						distanceMatrix[indexA - 1][indexB],
						distanceMatrix[indexA][indexB - 1],
						distanceMatrix[indexA - 1][indexB - 1],
					);
			}
		}
	}
