...
import {
	buildReviewCommentMarkdown,
	ensurePullRequestIssue,
	fetchParentReviewComment,
	formatReviewThreadContext,
} from "./pull-request-review-utils";

export async function updateReviewComment(
	context: Context<"pull_request_review_comment.edited">,
) {
	const {
		logger,
		adapters: { supabase },
	...
	if (!isHumanAuthor && !isThreadRoot) {
		logger.debug("Ignoring review comment update from non-human author", {
	...
	if (!isHumanAuthor && isThreadRoot) {
		logger.debug(
			"Allowing bot-authored root review comment update for thread context",
			{
				author: comment.user?.login,
				type: comment.user?.type,
	...
	try {
		...
		if (!pullRequest) {
			logger.warn(
				"Pull request payload missing; cannot update review comment",
				{ commentId: comment.id },
			);
			return;
		}
		...
		if (parentId) {
			const parentContext = await fetchParentReviewComment(
				context,
				owner,
				repo,
				parentId,
			...
			if (parentContext) {
				finalMarkdown = formatReviewThreadContext(
					enrichedComment,
					parentContext.markdown,
					parentContext.author,
				);
			}
		}
		...
		if (config.demoFlag) {
			logger.debug(
				"Demo mode active - skipping review comment update in database",
				{ comment: comment.id, comment_url: comment.html_url },
			);
			return;
		}
		...
		if (error instanceof Error) {
			logger.error("Error updating review comment", {
				error,
				stack: error.stack,
				comment,
			});
