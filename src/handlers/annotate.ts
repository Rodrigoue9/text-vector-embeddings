...
import {
	processSimilarIssues,
	IssueGraphqlResponse,
	findMostSimilarSentence,
} from "./issue-deduplication";
...
export async function commentChecker(
	...
	if (similarIssues && similarIssues.length > 0) {
		processedIssues = await processSimilarIssues(
			similarIssues,
			context,
			commentBodyForMatching,
		);
		...
		if (processedIssues.length > 0) {
			logger.info(
				`Similar issue which matches more than ${context.config.annotateThreshold} already exists`,
				{ processedIssues },
			);
		} else {
			context.logger.info("No similar issues found for comment", {
				commentBody,
			});
		}
	} else {
	...
	if (similarComments && similarComments.length > 0) {
		processedComments = await processSimilarComments(
			similarComments,
			context,
			commentBodyForMatching,
		);
		...
		if (processedComments.length > 0) {
			logger.info(
				`Similar comment which matches more than ${context.config.annotateThreshold} already exists`,
				{ processedComments },
			);
		} else {
			context.logger.info("No similar comments found for comment", {
				commentBody,
			});
		}
	} else {
	...
	await handleSimilarIssuesAndComments(
		context,
		payload,
		commentBody,
		comment.id,
...
function filterByScope(
	...
	switch (scope) {
		...
		case "repo":
			return (
				repoOrg === similarIssueRepoOrg && repoName === similarIssueRepoName
			);
		default:
			return false;
	}
...
async function handleSimilarIssuesAndComments(
	...
	issueList.forEach((issue, index) => {
		...
		footnotes.push(
			`${footnoteRef}: ${issue.similarity}% similar to issue: [${issue.node.title}](${modifiedUrl}#${issue.node.number})\n\n`,
		);
	});
	...
	commentList.forEach((comment, index) => {
		const footnoteIndex = highestFootnoteIndex + index + 1; // Continue numbering from the highest existing footnote number
		const footnoteRef = `[^0${footnoteIndex}^]`;
		const modifiedUrl = comment.node.url.replace(
			"https://github.com",
			"https://www.github.com",
		);
		...
		// Add new footnote to the array
		footnotes.push(
			`${footnoteRef}: ${comment.similarity}% similar to comment: [#${comment.node.issue.number} (comment)](${modifiedUrl})\n\n`,
		);
	});
...
async function processSimilarComments(
	...
	const processedComments = await Promise.all(
		similarComments.map(async (comment: CommentSimilaritySearchResult) => {
			try {
				const commentUrl: CommentGraphqlResponse =
          ...
          `,
				...
				commentUrl.mostSimilarSentence = findMostSimilarSentence(
					commentBody,
					similarBody,
					context,
				);
				...
				context.logger.error(
					`Failed to fetch comment ${comment.comment_id}: ${error}`,
					{ comment },
				);
				return null;
			}
	...
	return processedComments.filter(
		(comment): comment is CommentGraphqlResponse => comment !== null,
	);
}
...
export function removeAnnotateFootnotes(content: string): string {
	...
	if (footnotes) {
		footnotes.forEach((footnote) => {
			...
			contentWithoutFootnotes = contentWithoutFootnotes.replace(
				new RegExp(`\\[\\^${footnoteNumber}\\^\\]`, "g"),
				"",
			);
		});
	}
