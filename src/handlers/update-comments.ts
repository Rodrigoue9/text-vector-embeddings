...
import {
	checkIfAnnotateFootNoteExists,
	removeAnnotateFootnotes,
} from "./annotate";
...
export async function updateComment(context: Context<"issue_comment.edited">) {
	...
	if (payload.comment.user?.type !== "User") {
		logger.debug("Ignoring comment update from non-human author", {
	...
	try {
		...
		if (isPullRequestComment) {
			logger.debug(
				"Issue comment update is on a pull request; linking to PR document",
				{
			...
			issueId =
				(await ensurePullRequestIssue(context, payload.issue)) ?? issueId;
		} else {
		...
		if (config.demoFlag) {
			logger.debug("Demo mode active - skipping comment update in database", {
		...
		logger.ok(
			`Successfully updated comment! ${payload.comment.id}`,
			payload.comment,
		);
	} catch (error) {
