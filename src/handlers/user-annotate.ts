...
function buildRecommendationComment(
	...
	const formattedLogins = requestedLogins
		.map((login) => `@${login}`)
		.join(", ");
	const lines: string[] = [
		">[!NOTE]",
		requestedLogins.length > 0
			? `>Recommendation results (filtered): ${formattedLogins}`
...
async function postCommandResponse(
	...
	const options = forceTag
		? { raw: true, commentKind: "command-response" }
		: { raw: true };
...
export async function userAnnotate(context: Context<"issue_comment.created">) {
	...
	if (commandName === "recommendation") {
		...
		const result =
			requestedLogins.length > 0
				? await issueMatchingForUsers(context, requestedLogins)
		...
		await postCommandResponse(
			context,
			buildRecommendationComment(result, requestedLogins),
			true,
		);
	}
}
