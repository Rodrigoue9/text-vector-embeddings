...
export async function updatePullRequestReview(
	context: Context<"pull_request_review.edited">,
) {
	const {
		logger,
		adapters: { supabase },
	...
	if (review.user?.type !== "User") {
		logger.debug("Ignoring pull request review update from non-human author", {
	...
	try {
		if (!pullRequest) {
			logger.warn(
				"Pull request payload missing; cannot update review summary",
				{ reviewId: review.id },
			);
			return;
		}
		...
		const reviewMarkdown = buildPullRequestReviewMarkdown(
			{ body: cleanedBody, state: review.state ?? null },
			pullRequest,
		);
		if (!reviewMarkdown) {
		...
		if (config.demoFlag) {
			logger.debug(
				"Demo mode active - skipping pull request review update in database",
				{ review: review.id, review_url: review.html_url },
			);
			return;
		}
		...
		if (error instanceof Error) {
			logger.error("Error updating pull request review summary", {
				error,
				stack: error.stack,
				review,
			});
