...
export async function addReviewComment(
	...
	if (!isHumanAuthor && !isThreadRoot) {
		logger.debug("Ignoring review comment from non-human author", {
	...
	if (!isHumanAuthor && isThreadRoot) {
		logger.debug(
			"Allowing bot-authored root review comment for thread context",
			{
				author: comment.user?.login,
				type: comment.user?.type,
	...
	try {
		...
		if (!pullRequest) {
			logger.warn("Pull request payload missing; cannot store review comment", {
				commentId: comment.id,
			});
			return;
		}
		...
		if (config.demoFlag) {
			logger.debug("Demo mode active - skipping review comment storage", {
				comment: comment.id,
