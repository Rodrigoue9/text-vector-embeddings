...
export async function addComments(context: Context<"issue_comment.created">) {
	...
	if (comment.user?.type !== "User") {
		logger.debug("Ignoring comment from non-human author", {
	...
	try {
		...
		if (isPullRequestComment) {
			logger.debug(
				"Issue comment is on a pull request; linking to PR document",
				{ commentId: comment.id, pullRequestUrl: payload.issue.html_url },
			);
			issueId =
		...
		if (config.demoFlag) {
			logger.debug("Demo mode active - skipping comment storage", {
				comment: comment.id,
