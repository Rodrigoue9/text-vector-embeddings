...
export async function addIssue(
	...
	let markdown =
		payload.issue.body && payload.issue.title
			? `${payload.issue.body} ${payload.issue.title}`
			: null;
	...
	if (!isHumanAuthor) {
		logger.debug(
			"Issue author is not human; storing issue without embeddings.",
			{
	...
	try {
		...
		const cleanedIssue =
			isHumanAuthor && markdown ? await cleanContent(context, markdown) : null;
		const queueSettings = getEmbeddingQueueSettings(context.env);

		if (config.demoFlag) {
			logger.debug("Demo mode active - skipping issue storage", {
				issue: issue.number,
		...
		await supabase.issue.createIssue(
			{ id, payload, isPrivate, markdown: cleanedIssue, author_id: authorId },
			{ deferEmbedding: queueSettings.enabled },
		);
		if (isHumanAuthor) {
			await issueStore.addIssue(issue.html_url);
		}
