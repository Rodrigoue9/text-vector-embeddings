...
import {
	appendPluginUpdateComment,
	normalizeWhitespace,
	stripHtmlComments,
	stripPluginUpdateComments,
} from "../utils/markdown-comments";
import {
	appendFootnoteRefsToFirstLine,
	insertFootnoteRefNearSentence,
} from "../utils/footnote-placement";
 ...
 **/
export async function issueDedupe(
	context: Context<"issues.opened" | "issues.edited">,
	options: { keepUpdateComment?: boolean } = {},
) {
	const {
		logger,
		adapters: { supabase },
	...
	const { cleaned: bodyWithoutPluginUpdates, latestComment } =
		stripPluginUpdateComments(issueBody);
	const updateComment = shouldKeepUpdateComment ? latestComment : null;
	const cleanedIssueBody = await cleanContent(
		context,
		bodyWithoutPluginUpdates,
	);
	...
	if (similarIssues && similarIssues.length > 0) {
		let processedIssues = await processSimilarIssues(
			similarIssues,
			context,
			issueBodyForMatching,
		);
		processedIssues = processedIssues.filter((issue) =>
			matchRepoOrgToSimilarIssueRepoOrg(
				payload.repository.owner.login,
				issue.node.repository.owner.login,
				payload.repository.name,
				issue.node.repository.name,
			),
		);
		const matchIssues = processedIssues.filter(
			(issue) =>
				parseFloat(issue.similarity) / 100 >=
				context.config.dedupeMatchThreshold,
		);
		if (matchIssues.length > 0) {
			logger.info(
				`Similar issue which matches more than ${context.config.dedupeMatchThreshold} already exists`,
				{ matchIssues },
			);
			//To the issue body, add a footnote with the link to the similar issue
			const updatedBody = await handleMatchIssuesComment(
				context,
				payload,
				cleanedIssueBody,
				processedIssues,
			...
			const nextBody = updateComment
				? appendPluginUpdateComment(outputBody, updateComment)
				: outputBody;
			const isBodyUnchanged =
				normalizeWhitespace(originalIssue.body ?? "") ===
				normalizeWhitespace(nextBody);
			const shouldClose =
				originalIssue.state !== "closed" ||
				originalIssue.state_reason !== "not_planned";
			if (isBodyUnchanged && !shouldClose) {
				logger.info("Issue body unchanged after dedupe match update", {
		...
		if (processedIssues.length > 0) {
			logger.info(
				`Similar issue which matches more than ${context.config.dedupeWarningThreshold} already exists`,
				{ processedIssues },
			);
		...
		if (checkIfDuplicateFootNoteExists(cleanedIssueBody || "")) {
			const outputBody = updateComment
				? appendPluginUpdateComment(cleanedIssueBody, updateComment)
				: cleanedIssueBody;
			if (
				normalizeWhitespace(originalIssue.body ?? "") ===
				normalizeWhitespace(outputBody)
			) {
				logger.info("Issue body unchanged after duplicate footnote cleanup", {
					issueNumber: originalIssue.number,
				});
				return;
			}
...
function matchRepoOrgToSimilarIssueRepoOrg(
	repoOrg: string,
	similarIssueRepoOrg: string,
	repoName: string,
	similarIssueRepoName: string,
): boolean {
...
async function handleSimilarIssuesComment(
	...
	if (relevantIssues.length === 0) {
		context.logger.info(
			"No relevant issues found with the same repository and organization",
		);
		return;
	}
	...
	const highestFootnoteIndex =
		existingFootnotes.length > 0
			? Math.max(
					...existingFootnotes.map((fn) =>
						parseInt(fn.match(/\d+/)?.[0] ?? "0"),
					),
				)
			: 0;
	...
	relevantIssues.sort(
		(a, b) => parseFloat(a.similarity) - parseFloat(b.similarity),
	);
	relevantIssues.forEach((issue, index) => {
		...
		const modifiedUrl = issue.node.url.replace(
			"https://github.com",
		...
		if (!sentence.trim()) {
			...
			if (beforeReplace === updatedBody) {
				const fallback = insertFootnoteRefNearSentence(
					updatedBody,
					sentence,
					footnoteRef,
				);
		...
		footnotes.push(
			`${footnoteRef}: ⚠ ${issue.similarity}% possible duplicate - [${issue.node.title}](${modifiedUrl}#${issue.node.number})\n\n`,
		);
	});
	...
	const outputBody = latestComment
		? appendPluginUpdateComment(updatedBody, latestComment)
		: updatedBody;
	if (normalizeWhitespace(currentBody) === normalizeWhitespace(outputBody)) {
		context.logger.info("Issue body unchanged after dedupe warning update", {
			issueNumber,
		});
		return;
	}
...
async function handleMatchIssuesComment(
	...
	const footnoteIndex = existingFootnotes[0]
		? issueBody.indexOf(existingFootnotes[0])
		: issueBody.length;
	let resultBuilder =
	...
	// Update the issue with the modified body
	return (
		issueBody.slice(0, footnoteIndex) +
		resultBuilder +
		issueBody.slice(footnoteIndex)
	);
}

// Process similar issues and return the list of similar issues with their similarity scores
export async function processSimilarIssues(
	similarIssues: IssueSimilaritySearchResult[],
	context: Context,
	issueBody: string,
): Promise<IssueGraphqlResponse[]> {
	const processedIssues = await Promise.all(
		similarIssues.map(async (issue: IssueSimilaritySearchResult) => {
          ...
          `,
				...
				issueUrl.mostSimilarSentence = findMostSimilarSentence(
					issueBody,
					similarBody,
					context,
				);
				...
				context.logger.error(
					`Failed to fetch issue ${issue.issue_id}: ${error}`,
					{ issue },
				);
				return null;
			}
	...
	return processedIssues.filter(
		(issue): issue is IssueGraphqlResponse => issue !== null,
	);
}
...
async function handleAnchorAndImgElements(context: Context, content: string) {
	...
	async function processElement(
		...
		try {
			...
			if (!contentType?.startsWith("image/")) {
				context.logger.warn(
					`Content type is not an image: ${contentType}, will skip ${url}`,
				);
				return;
			}
	...
	context.logger.debug("Enriched comment content with image descriptions.", {
		content,
	});
	return content;
}
...
export async function cleanContent(
	...
	return stripDuplicateFootnotes(
