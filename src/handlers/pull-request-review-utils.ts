...
export function buildPullRequestReviewMarkdown(
...
export function buildReviewCommentMarkdown(
	comment: ReviewCommentPayload,
): string | null {
...
export function formatReviewThreadContext(
	base: string,
	parent: string,
	...
	try {
		...
		if (!isHumanAuthor && !isThreadRoot) {
			context.logger.debug(
				"Skipping parent review comment from non-human author",
				{
					parentId,
		...
		if (!isHumanAuthor && isThreadRoot) {
			context.logger.debug(
				"Including bot-authored root review comment for thread context",
				{
					parentId,
					author: data.user?.login ?? null,
				},
...
export async function ensurePullRequestIssue(
	...
	if (!isHumanAuthor) {
		logger.debug(
			"Pull request author is not human; storing issue without embeddings.",
			{
	...
	if (isHumanAuthor && !markdown) {
		logger.warn(
			"Pull request title/body is empty; skipping PR issue creation",
			{ pullRequestUrl: pullRequest.html_url },
		);
		return id;
	}
	...
	if (config.demoFlag) {
		logger.debug("Demo mode active - skipping PR issue storage", {
			pullRequestUrl: pullRequest.html_url,
		});
		return id;
	}
	...
	logger.info("Created PR issue entry for review comment linkage", {
		id,
		pullRequestUrl: pullRequest.html_url,
	});
	return id;
