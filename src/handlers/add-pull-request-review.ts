...
export async function addPullRequestReview(
	...
	if (review.user?.type !== "User") {
		logger.debug("Ignoring pull request review from non-human author", {
	...
	try {
		if (!pullRequest) {
			logger.warn("Pull request payload missing; cannot store review summary", {
				reviewId: review.id,
			});
			return;
		}
		...
		if (config.demoFlag) {
			logger.debug("Demo mode active - skipping pull request review storage", {
				review: review.id,
		...
		if (error instanceof Error) {
			logger.error("Error creating pull request review summary", {
				error,
				stack: error.stack,
				review,
			});
