...
export class PostgresRateLimitStore implements Store {
	...
	async increment(key: string): Promise<ClientRateLimitInfo> {
		...
		const payload: ClientRateLimitInfo = {
			...
			resetTime:
				current?.resetTime ??
				new Date(nowMs + (this._options?.windowMs ?? 60000)),
		};
	...
	private async _getActiveRecord(
		key: string,
		nowMs: number,
	): Promise<ClientRateLimitInfo | undefined> {
	...
	private async _upsertRecord(
		...
		const resetTime =
			payload.resetTime ??
			new Date(Date.now() + (this._options?.windowMs ?? 60000));
